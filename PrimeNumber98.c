...
"Pi(1e15, 1e10) = 289531946    10.0   5.10    4.32     3.67    3.91\n"
"Pi(1e16, 1e10) = 271425366    11.8   6.10    5.11     4.34    4.73\n"
"Pi(1e17, 1e10) = 255481287    14.0   7.09    5.95     5.17    5.63\n"
"Pi(1e18, 1e10) = 241272176    17.3   8.58    7.17     6.25    6.88\n"
...
"Pi(1e18, 10^9) = 24217085     3.50   1.80    1.58     1.40    1.50\n"
"Pi(0,    1e12) = 37607912018  500    270     224      200     220\n"
"Pi(1e14, 1e12) = 31016203073  790    420     354      295     320\n"
"Pi(1e16, 1e12) = 27143405794  1160   600     512      430     485\n"
"Pi(1e18, 1e12) = 24127637783  1500   760     622      560     640\n"
...
//medium prime
#if SM2
typedef struct SmallSieve MediumSieve;
#else
...
static void setWheelSmall(const uint64 start, const uint maxp)
{
	...
	for (p = SmallPrime[FIRST_INDEX].Sp, j = FIRST_INDEX; p < maxp; p = SmallPrime[++j].Sp) {
		...
		const int pi = WheelInit30[p % WHEEL].WheelIndex;
...
static void setWheelMedium(uchar* bitarray, const uint sieve_size, const uint medium, const uint64 start)
{
	...
	for (uint i = 0; i < pn; ) {
		...
		if (Threshold.L2Index == 0) {
			MediumPrime[j - 1].Sp = p;
			const uint multiples = (NEXT_MULTIPLE >> (wf.Multiple * 3)) | (NEXT_MULTIPLE << (24 - wf.Multiple * 3));
			MediumPrime[j - 1].Multiple = multiples;

#ifdef SM0
			MediumPrime[j - 1].Si = sieve_index;
#else
			MediumPrime[j - 1].Si = 0 - sieve_index;
#endif
		}
#endif
	}
...
static void eratSieveSmall1(uchar bitarray[], const uint64 start, const uint sieve_size, uint maxp)
{
	//assert(maxp < (1 << 16))
...
#if SM2
//sieve prime multiples in [start, start + sieve_size) with small algorithm
static void eratSieveSmall2(uchar bitarray[], const uint64 start, const uint sieve_size, const uint spi, uint maxp)
{
	//assert(maxp < (1 << 16))
	if (start + sieve_size < maxp * maxp)
		maxp = isqrt(start + sieve_size) + 1;
	...
	const uchar* pend = bitarray + sieve_size / WHEEL30;
	for (uint p = MediumPrime[spi].Sp, j = spi; p < maxp; p = MediumPrime[++j].Sp) {
		uint offset = MediumPrime[j].Si;
#ifdef SM0
		offset += crossSmall0(bitarray, pend, p, offset, MediumPrime[j].Multiple) + 30 * p - sieve_size;
#else
		if ((int)offset < 0) {
			offset = 0 - offset;
			for (int i = 0; i < 24; i += 3) {
				...
				bitarray[offset / WHEEL30] |= mask;
				offset += (MediumPrime[j].Multiple >> i) % 8 * p;
			}
		}
		offset += crossSmall3(bitarray + offset / WHEEL30, pend, p) - sieve_size;
#endif
		MediumPrime[j].Si = offset;
	}
}
#endif
...
{
	if (start + sieve_size < (long long)maxp * maxp)
		maxp = isqrt(start + sieve_size) + 1;
...
#ifndef W210
	const uint minsp = MIN(maxp, sieve_byte) / 30 << SI_BIT;
...
static void eratSieveL1(uchar bitarray[], const uint64 start, const uint sieve_size)
{
	...
	for (uint offset = 0, l1_size = Threshold.L1Size * WHEEL30; offset < sieve_size; offset += l1_size) {
		...
		eratSieveSmall1(bitarray + offset / WHEEL30, start + offset, l1_size, l1_maxp);
	}
}
...
static int segmentedSieve(uchar bitarray[], const uint64 start, const uint sieve_size)
{
	...
	const uint max_over  = bmsieve ? Threshold.L2Maxp : l1_maxp;
	for (uint i = 0; i < max_over; i += sizeof(uint)) {
	...
	for (uint offset = 0, l2_size = Threshold.L2Size * WHEEL30; offset < sieve_size; offset += l2_size) {
		...
		eratSieveL1(bitarray + offset / WHEEL30, start + offset, l2_size);
		if (bmsieve) {
#if SM2
			eratSieveSmall2(bitarray + offset / WHEEL30, start + offset, l2_size,  Threshold.L1Index, Threshold.L2Maxp);
#else
			eratSieveMedium1(bitarray + offset / WHEEL30, start + offset, l2_size, Threshold.L1Index, Threshold.L2Maxp);
#endif

#ifndef FB
		if (start >= Threshold.BucketStart)
			eratSieveBig(bitarray + offset / WHEEL30, 1 << BucketInfo.Log2Size);
#endif
		}
	}
	...
	for (uint offset = 0, l1_size = Threshold.L1Size * WHEEL30; offset < sieve_size; offset += l1_size) {
		...
		for (uint p = SmallPrime[FIRST_INDEX].Sp, j = FIRST_INDEX; p < maxp; p = SmallPrime[++j].Sp) {
			...
			if ((int)offset1 < 0)
				offset1 = 0 - offset1;
	#endif
			SmallPrime[j].Si = offset1 + crossSmall0(pstart, pend, p, offset1, SmallPrime[j].Multiple) + 30 * p - l1_size;
		}
	}
#endif
...
void setCacheSegs(uint level, uint segs)
{
	...
	if (level == 1) {
		...
		Config.Msegs = segs;
	} else if (level == 4 && segs <= 32 && segs > 0) {
...
static uint adjustSieveSize(uint sieve_size, const uint sqrtp)
{
	...
	int bsegs = sieve_size / Threshold.L2Size;
#if FB
	if (Threshold.L2Size >= sieve_size)
		Config.Bsegs = 1;
	else if (Config.Bsegs > bsegs)
		Config.Bsegs = bsegs;
#else
	if (bsegs == 0)
		bsegs = 1;
...
static void allocMedium(const uint medium)
{
	...
	if (MediumPrime == NULL) {
		...
		MediumPrime = (MediumSieve*) malloc(sizeof(MediumSieve) * (pix + PI_65536));
...
uint64 doSieve(const uint64 start, const uint64 end, PrimeCall* pcall)
{
	...
	const uint l1_maxp = Threshold.L1Maxp;
	const uint max_cache = MAX(sieve_size / WHEEL30, L2_DCACHE_SIZE * 1024) + Threshold.L2Maxp + sizeof(uint64);
	...
	if (sqrtp > medium) {
		assert(Threshold.L2Size <= sieve_size);
	...
	if (Config.Flag & PRINT_RET) {
		...
		if (Config.Flag & PRINT_TIME)
			printf(" (%.2f + %.2f = %.2f sec %u kb L%u%u%u %d)",
					(ti - ts) / 1000.0, (ta - ti) / 1000.0, (ta - ts) / 1000.0, Config.SieveSize >> 10, Config.L1Segs, Config.L2Segs, Config.Msegs, Config.Bsegs);
		putchar('\n');
	}
...
static void printInfo()
{
	...
	info += sprintf(info, "[MARCO] : MEM_WHEEL = %d mb, WHEEL_SIZE = %d kb, SIEVE_SIZE = %d kb, WHEEL = %d\n",
...
static void randTest()
{
	...
	for (int j = 1; j <= 10000; j ++) {
		...
		setCacheSegs(1, rand() % 8 + 1);
		setCacheSegs(2, rand() % 6 + 2);
